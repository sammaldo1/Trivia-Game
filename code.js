...
// YOUR CODE HERE!
//create 2 pages view One & view 2
//View One should be where the game starts
    //includes page title h1
    //start game button add text "Let's play"
    //after start game button is clicked should take to View 2
    //remove submit button and append everything else(one option), wrap button in a a<> with the html , or by using window.location.
//View 2:
//should display to HTML: 
    //title using h1
    //category using h2
    //total points using <p>
    //questaions using div id="questions"
    //answers box using input box with  div id="answers"
    //submit with button (creat an event listener that checks users answer)

//combining JS to HTML:
...
//keep track of and display a users score
    ...
    //will need to create button on page 1 for starting game, title(h1) for both pages,for second page:  category(h2), total points(p), q(ptag), a(input box), submit button(to submit a)


    //will need button for starting game:
        //button will be on first page, which takes to second page
        //will need event listener to take to next page

    //will need to use fetch to get categories and q's&a's
    ...
    //will need a button that allowes the code to rerender to start game play over again. 
    //will be called if incorrect answer is called. 

    //will need a if statement for totalScore 
